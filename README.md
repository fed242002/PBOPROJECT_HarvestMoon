...
Nanem Bulan implements an object-level binary persistence system built on native **Java Object Serialization** (`java.io.Serializable`), enabling players to save their session progress to disk and restore their character and world timeline upon restarting the application.

#### Architecture & Design Patterns
The persistence subsystem is housed under the `Data` package and employs two foundational software engineering patterns:
1. **Memento & Data Transfer Object (DTO) Pattern:** Rather than serializing complex, active runtime objects like `GamePanel` or `Player` directly (which contain graphical buffers, thread references, event listeners, and sprite assets that are non-serializable), [`dataStorage.java`](src/Data/dataStorage.java) acts as a clean, decoupled DTO snapshot holding purely persistent primitives and immutable strings.
2. **Facade & Service Pattern:** [`saveLoad.java`](src/Data/saveLoad.java) provides high-level facade methods (`save()` and `load()`) to the game engine, completely hiding low-level file I/O, byte stream buffering, and exception handling from UI controllers and input handlers.

```mermaid
sequenceDiagram
    autonumber
    actor Player
    participant UI as UI.java / KeyHandler
    participant GP as GamePanel.java
    participant SL as saveLoad.java
    participant DTO as dataStorage.java (DTO)
    participant Disk as File ("save.dat")

    Note over Player,Disk: Saving Progress (In-Game via Pause Menu)
    Player->>UI: Press Esc -> Navigate to "Save" -> Press Enter
    UI->>GP: Set pauseScreenState = 1
    UI->>SL: gp.saveLoad1.save()
    SL->>DTO: Instantiate new dataStorage()
    SL->>GP: Read player.energy, player.gold, hour, minute, timeCounter, player.name
    SL->>DTO: Pack state into DTO fields
    SL->>Disk: ObjectOutputStream.writeObject(ds) -> writes to save.dat
    SL-->>UI: Output "Progress has been saved" to stdout
    UI->>UI: Reset pauseScreenState = 0 (Return to Pause Menu)

    Note over Player,Disk: Loading Progress (Title Screen Menu)
    Player->>UI: Title Screen -> Select "LOAD GAME" -> Press Enter
    UI->>SL: gp.saveLoad1.load()
    SL->>Disk: ObjectInputStream.readObject() from save.dat
    Disk-->>SL: Returns deserialized dataStorage instance
    SL->>GP: Restore gp.player.energy = ds.energy
    SL->>GP: Restore gp.player.gold = ds.coin
    SL->>GP: Restore gp.hour = ds.hour, gp.minute = ds.minute
    SL->>GP: Restore gp.timeCounter = ds.timeCounter
    SL->>GP: Restore gp.player.name = ds.nama
    SL-->>UI: Output "Load Complete" to stdout
    UI->>GP: Transition gameState = gp.playState
```

#### Persisted Data Storage Schema
All session data written to `save.dat` is defined within [`dataStorage.java`](src/Data/dataStorage.java):

| Field Name | Data Type | Engine Source | Serialization Mapping | In-Game Significance & Runtime Restoration |
| :--- | :---: | :--- | :--- | :--- |
| **`energy`** | `int` | `gp.player.energy` | `ds.energy` | Current player stamina reserve ($0..100$). Restores directly into `EnergyBar` for immediate stamina availability. |
| **`coin`** | `int` | `gp.player.gold` | `ds.coin` | Liquid currency possessed by the player. Restores the player's wallet balance for buying seeds, livestock, and supplies. |
| **`nama`** | `String` | `gp.player.name` | `ds.nama` | Character name chosen during title screen setup. Restores player identity in dialogue boxes and menus. |
| **`hour`** | `int` | `gp.hour` | `ds.hour` | Current 24-hour clock hour ($0..23$). Instantly recalculates ambient light alpha (`Lighting.java`) and day/night transitions. |
| **`minute`** | `int` | `gp.minute` | `ds.minute` | Current clock minute ($0..59$). Restores exact clock positioning on the in-game HUD. |
| **`timeCounter`** | `int` | `gp.timeCounter` | `ds.timeCounter` | Sub-minute delta tick accumulator ($0..1800$ frames at 60 FPS). Ensures frame-accurate clock synchronization without losing fractional minute progress. |
| **`name`** | `String` | Reserved | (DTO Field) | Reserved legacy field in `dataStorage` maintained for serialized data structure backward compatibility. |

#### Detailed Lifecycle Execution

##### 1. The Save Routine (`saveLoad.save()`)
- **Trigger Condition:** Triggered when the player pauses the game with `Esc` and presses Enter on the "Save" entry (`pauseScreenState == 1`).
- **Resource Management:** Employs Java 7+ **try-with-resources** syntax (`try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(new File("save.dat"))))`), ensuring underlying OS file descriptors and stream buffers are flushed and closed automatically even if an unhandled I/O runtime exception occurs.
- **Fail-Safe Exception Handling:** File access errors (such as read-only disk permissions or storage exhaustion) are intercepted within a `catch (Exception e)` block, emitting `"Save Exception"` with stack tracing to stderr without causing game crashes or unhandled Swing thread terminations.
- **UI State Machine Reset:** Immediately after `save()` completes, `gp.ui.pauseScreenState` is reset to `0`, seamlessly reverting the UI back to the primary pause screen view.

##### 2. The Load Routine (`saveLoad.load()`)
- **Trigger Condition:** Executed on the Title Screen (`titleState`, `titleScreenState == 0`) when the player selects "LOAD GAME" (menu command index `1`).
- **Object Deserialization:** Reads binary data via `ObjectInputStream`, casts the deserialized instance back to `dataStorage`, and injects state values back into active game objects (`gp.player` and `gp`).
- **Instant Gameplay Transition:** Following deserialization, the engine immediately sets `gp.gameState = gp.playState`, bypassing the character creation and preview states and dropping the player straight into the active farm world.
- **Missing Save Guard:** If `save.dat` does not exist (e.g. fresh installation), `FileInputStream` throws a `FileNotFoundException`, caught cleanly by the exception block (`"Load Exception"`), preventing the application from freezing.

#### Source Code Implementation

The full data persistence logic is concise, thread-safe, and self-contained:
    ...
    public void load() {
        try {
            try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(new File("save.dat")))) {
                // Read and cast serialized object
                dataStorage ds = (dataStorage) ois.readObject();

                // Restore live game engine variables
...
#### Binary Storage File Specifications
- **Storage Path:** `./save.dat` (saved directly in the current runtime working directory).
- **Magic Byte Header:** `0xAC 0xED 0x00 0x05` (standard Java Serialization protocol identifier).
- **Storage Footprint:** Extremely lightweight (~130 to ~3,200 bytes), allowing instant sub-millisecond read/write cycles without frame drops on the 60 FPS loop.
- **Starting Fresh / Save Reset:** Players can reset their save data at any time by simply deleting `save.dat` from the game root folder:
  ```bash
  # Windows
  del save.dat
...
#### 🛠 Extension Guidelines: Adding New Persistent Fields
The codebase includes the internal developer directive `// tambah sendiri bang` (Indonesian: *"add additional fields yourself"*), making the persistence system easily expandable for future development:

1. **Player Coordinates & Current Map:**
   ```java
   // In dataStorage.java:
   int worldX, worldY, currentMap;

   // In saveLoad.java save():
   ds.worldX = gp.player.worldX;
   ds.worldY = gp.player.worldY;
   ds.currentMap = gp.currentMap;

   // In saveLoad.java load():
   gp.player.worldX = ds.worldX;
   gp.player.worldY = ds.worldY;
   gp.currentMap = ds.currentMap;
   ```
2. **Inventory Persistence:**
   Add an `ArrayList<String> itemNames` and `ArrayList<Integer> itemAmounts` to `dataStorage.java` to serialize backpack items and quantities across runs.
3. **Farm & Crop Map States:**
   Serialize coordinates and growth stages (`Crop.growthStage`, `OBJ_soil.tilledState`) into serializable arrays to preserve field layout and crop growth between game restarts.
4. **Serialization Versioning:**
   Always maintain a `private static final long serialVersionUID` in `dataStorage.java` to prevent `InvalidClassException` across differing Java compiler versions.
